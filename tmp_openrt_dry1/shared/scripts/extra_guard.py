...
def _coerce_validation_cases(candidate: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    positive = (
        ...
        or candidate.get("validation_positive_cases")
        or candidate.get("positive_validation_cases")
        or []
    )
    negative = (
        raw_cases.get("negative")
        or raw_cases.get("negative_cases")
        or candidate.get("validation_negative_cases")
        or candidate.get("negative_validation_cases")
        or []
    )
    return {
        "positive": _clean_validation_list(positive),
        "negative": _clean_validation_list(negative),
    }
